...
/**
 * TODO: Implement UDPSplitHandler Netty In/Out Handler that splits/regroups
 * messages into/from multiple UDP packets due to maximum packet length
 * limitations
 * 
 * 
 * 
...
public class UDPSplitHandler implements ChannelUpstreamHandler, ChannelDownstreamHandler {
	...
	public void handleDownstream(ChannelHandlerContext ctx, ChannelEvent e) throws Exception {
		...
		int dataSize = cb.readableBytes() < (maxPacketSize - FIRST_MESSAGE_HEADER_SIZE) ? cb.readableBytes()
				: (maxPacketSize - FIRST_MESSAGE_HEADER_SIZE);
		...
		while (cb.readable()) {
			...
			dataSize = cb.readableBytes() < (maxPacketSize - NEXT_MESSAGE_HEADER_SIZE) ? cb.readableBytes()
					: (maxPacketSize - NEXT_MESSAGE_HEADER_SIZE);
			...
			// System.out.println("Sent: " + ((short) lastSentMessageId) + "," +
			// packet);
