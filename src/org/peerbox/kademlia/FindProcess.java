 ...
 * 
 * @param <FRT>
 *            the type of FindResponse FindNodeResponse or a FindValueResponse
 */
public class FindProcess<FRT extends FindResponse> {
	...
	protected final Set<Node> nearestSet; // nodes that have been found through
											// all searches
	protected final Set<Node> current; // nodes currently being queried but
										// haven't replied
	...
	// TODO: a FindProcessResponse<FRT> object should be sent to the callback to
	...
	private FindProcess(NetworkInstance ni, FindRequest request, boolean stopOnFound, Class<FRT> responseClass,
		...
		nearestSet = Collections.synchronizedSet(new TreeSet<Node>(new IdentifiableDistanceComparator(findRequest
				.getTargetIdentifier())));
		nearestSet.addAll(networkInstance.getBuckets()
				.getNearestNodes(findRequest.getTargetIdentifier(), searchSetSize));
	...
	private void nextIteration(final LinkedList<Node> unsearchedNodes) {
		...
		while (current.size() < maxRequests && !unsearchedNodes.isEmpty()) {
			...
			networkInstance.sendRequestRPC(nextRequestDestination, findRequest, responseClass,
					new ResponseListener<FRT>() {
						// event that a message was received
						public void onResponseReceived(FRT response) {
							// if the reply contains the target then trigger
							// identifier found
							...
							// otherwise add the nodes to the nearestSet and to
							// the k-buckets
							else {
	...
	private void attemptDone(LinkedList<Node> unsearchedNodes) {
		if (!done) {
			// if no new unsearched nodes and queue is over means the requested
			// value wasn't found
			if (unsearchedNodes.isEmpty() && current.isEmpty()) {
				FRT response = foundResponse == null ? lastResponse : foundResponse;
				if (response != null) {
					// if there has been at least one response and no more
					// unsearched nodes
	...
	private LinkedList<Node> computeUnsearchedNodes() {
		 ...
		 */
		// If we iterate over nearest set, remember to do so in a synchronized
		// block
		// synchronized(nearestSet) { }

		// find the unsearched nodes by removing the previously queried from the
		// nearest set
